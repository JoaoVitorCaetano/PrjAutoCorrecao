...
class Montadora:
   ...
   def __init__(self, sgl_montadora='', nme_montadora='', idt_montadora=None):
       ...
       self._modelos = None # Atributo interno para armazenar os modelos da Montadora corrente
   ...
   def modelos(self):
       if self._modelos is None:
           ...
           dao = ModeloDAO()
           self._modelos = dao.selecionar_por_montadora(self.idt_montadora)
...
class MontadoraDAO:
   ...
   def selecionar_tudo(self):
       ...
       sql = "SELECT idt_montadora, sgl_montadora, nme_montadora FROM tb_montadora ORDER BY sgl_montadora, nme_montadora"
   ...
   def selecionar_por_idt(self, idt):
       # Selecionar uma montadora por idt
       if not self.conexao:
           print("Erro: Nenhuma conexão com o banco de dados.")
           return None


       sql = "SELECT idt_montadora, sgl_montadora, nme_montadora FROM tb_montadora WHERE idt_montadora = %s"
       ...
       try:
           ...
           if resultado is None:
               ...
               montadora = Montadora(idt_montadora=resultado[0], sgl_montadora=resultado[1], nme_montadora=resultado[2])
               return montadora
       except mysql.connector.Error as err:
           print(f"Erro ao selecionar montadora: {err}")
           return None
